...
void ADXL345::powerOn() {
	if(I2C) {
			...
			Wire.begin(PIN_I2C_SDA, PIN_I2C_SCL);				// If in I2C Mode Only
			grbl_msg_sendf(CLIENT_SERIAL, MsgLevel::Info, "I2C on pin SDA:%s SCL:%s", pinName(PIN_I2C_SDA).c_str(), pinName(PIN_I2C_SCL).c_str());
		#else
			Wire.begin();
		#endif
	}
...
void ADXL345::readFrom(byte address, int num, byte _buff[]) {
	if(I2C) {
		...
		// grbl_msg_sendf(CLIENT_SERIAL, MsgLevel::Info, "I2C ");
